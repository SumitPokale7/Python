...
LAMBDA_NAME="INTERIM-${HUB_ACCOUNT_NAME}-LMD-MIGRATE-SPOKES-TO-CONTROL-TOWER"
ROLE_NAME="INTERIM-${HUB_ACCOUNT_NAME}-role_MIGRATE-SPOKES-TO-CONTROL-TOWER"
ROLE_ARN="arn:aws:iam::${HUB_ACCOUNT_ID}:role/$ROLE_NAME"
POLICY_NAME="INTERIM-CT-MIGRATION-LMD-inline-policy"
...
aws iam create-role \
    --role-name $ROLE_NAME \
    --assume-role-policy-document '{"Version": "2008-10-17", "Statement": [{"Effect": "Allow", "Principal": {"Service": "lambda.amazonaws.com"}, "Action": "sts:AssumeRole"}]}'

echo ">> Creating lambda role policy"
aws iam put-role-policy \
    --role-name $ROLE_NAME \
    --policy-name $POLICY_NAME \
    --policy-document file://migration-lambda-trust-policy.json

echo ">> Waiting for role to propagate"
for i in {0..9}; do echo "Waiting $((10-$i))"; sleep 1; done

# Create a Lambda function
echo '>> creating lambda function...'
aws lambda create-function \
    --function-name $LAMBDA_NAME \
    --description 'Temporary Lambda to migrate Spokes from Landing Zone to Control Tower' \
    --zip-file fileb://build/dist.zip \
    --handler migrate-spokes-to-control-tower.lambda_handler \
    --runtime python3.8 \
    --timeout 900 \
