...
def create_creds(role, region):
    ...
    return sts_client.assume_role(
        RoleArn=role, RoleSessionName="public-subnets-standalone-accounts"
    )
        ...
        for account in list_accounts:
            ...
            for instance in describe_instances["Reservations"]:
                for image in instance["Instances"]:
                    key = "SubnetId"
                    if key in image and image[key] in subnet_list_in_account:
...
def get_connected_spokes(environment_type):
    ...
    params = {
        ...
        "ProjectionExpression": "#c, account, #at",
