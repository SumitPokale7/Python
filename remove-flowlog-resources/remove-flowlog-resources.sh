#!/bin/bash
#Script to remove vpc flowlog buckets and flowlog retention lambda log group from spoke accounts
set -e
...
for spoke in $active_spokes; do
    ...
    then
        ...
        temp_role=$(aws sts assume-role \
                        ...
                        --role-session-name "FlowLogBucketDeletion"\
        ...
        #removing LogReplicationFlowLog lambda log group
        log_group=$(aws logs describe-log-groups --log-group-name-prefix "/aws/lambda/${spoke}-LMD_LogReplicationVPCFlow" | jq -r '.logGroups[].logGroupName')
        if [[ -z "$log_group" ]]
        then
        ...
        #removing Flowlog group
        flowlog_group=$(aws logs describe-log-groups --log-group-name-prefix "/aws/vpc/${spoke}-VPCFlowLogs" | jq -r '.logGroups[].logGroupName')
        if [[ -z "$flowlog_group" ]]
        then
            echo "flowLog group doesn't exist for: ${spoke}"
        else
            aws logs delete-log-group --log-group-name $flowlog_group
            echo "flowLog group deleted for: ${spoke}"
        fi

        # removing cloudtrail buckets from spokes
        list_buckets=$(aws s3 ls)
        if [[ $list_buckets =~ "${spoke}-cip-logs-vpcflow" ]]
        then
            echo "Bucket exist for: ${spoke}"
            aws s3 rb s3://${spoke}-cip-logs-vpcflow --force
        else
            echo "Bucket was already deleted for ${spoke} account."
        fi
        accounts_completed=$((${accounts_completed}+1))
        echo "spokes completed = ${accounts_completed}"
