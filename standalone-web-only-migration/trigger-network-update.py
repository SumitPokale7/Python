...
def main(hub_env, hub_account_id):
    try:
        # Update the spoke list with the accounts you want to trigger network update
        spoke_list = []
        for spoke in spoke_list:
            spoke_info = get_spoke_id(spoke_name=spoke, hub_env=hub_env)
...
def get_spoke_id(spoke_name, hub_env):
    table_name = f"{hub_env}-DYN_METADATA"
    client = boto3.client("dynamodb", "eu-west-1")
    logger.info(f"getting account id for spoke {spoke_name} from Metadata table.")
    response = client.get_item(
        TableName=table_name,
        Key={"account-name": {"S": spoke_name}},
    )
    logger.info(f"account id is {response['Item']['account']['S']} ")
